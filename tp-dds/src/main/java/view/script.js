...
function ejecutarControllers() {
    ...
    $.get('http://localhost:8080/diseno-de-sistemas/add-user?name=mauri&password=mauri&type=ADMIN&action=ADDTERMINAL,ADDPOI&resolutionType=Email', function (dataReceived) {
...
function eliminarPermiso(target) {
    ...
    for(j;j<actionsSize;j++){
        ...
        if(target.textContent==(currenAction)){
            i=1;
            break;
        }
    }
    ...
    if(i==1){
        ...
        var index= currentUser.actions.indexOf(target.textContent);
        currentUser.actions.splice(index, 1);
...
function Ok() {
    ...
    var res = '';
    var sizeActions =  users[id].actions.length;


    for (i = 0; i < sizeActions; i++) {
        text = users[id].actions[i];
    ...
    var actionObj= {};
    users[id].actions=[];
    actionObj.action=res;
    action.push(actionObj);
    users[id].actions.push(actionObj);
...
function selectUser() {
    ...
    var idn = users[userSelected].actions[users[userSelected].actions.length-1].action;

    agregarGraficaPermiso(idn, "#usuarios", "#permisos",userSelected);
...
function agregarGraficaPermiso(id, pathOuter, pathInner,userSelected) {
    ...
    users[userSelected].actions=action;
}
